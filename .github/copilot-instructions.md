...
- Only keep scraper/index changes when they do not reduce `Total Products`, do not worsen `Missing RAM` or `Missing SSD`, and do not introduce more implausible RAM values. For Copilot-created fix PRs, prefer a measurable improvement.
- Run `python3 -m py_compile scraper/scraper.py scraper/verify_data.py scripts/metrics_gate.py` before finishing.
